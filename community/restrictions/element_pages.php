...
class ElementPages extends \Modules\developer\std_mod\Element { //data element in area
    ...
    function __construct($variables) {
        ...
        $this->recordToOptionTable = 'm_community_restrictions_to_page';
        $this->recordToOptionRecordReference = 'ruleId';
    ...
    function printFieldNew($prefix, $parentId, $area) {
        ...
        foreach ($this->options as $languageKey => $language) {
            ...
            $html->html('<span class="label"><strong>' . htmlspecialchars($language['title']) . '</strong></span><br />');
            foreach ($language['zones'] as $zoneKey => $zone) {
                ...
                $html->html('<span class="label"><strong>' . htmlspecialchars($zone['title']) . '</strong></span><br />');
                ...
                foreach ($zone['options'] as $optionKey => $value) {
                    ...
                    $label = str_replace('—','',$value[1],$level);
                    $indent = '';
                    for ($i = 1; $i <= $level; $i++) {
                        $indent .= '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
                    }
                    $html->html($indent.'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" class="stdModBox" name="' . htmlspecialchars($name) . '" />');
                    $html->html('<span class="label">' . htmlspecialchars($label) . '</span>');
                    $html->html('<br />');
                }
            }
        }
    ...
    function printFieldUpdate($prefix, $record, $area) {
        ...
        foreach ($this->options as $languageKey => $language) {
            ...
            $html->html('<br /><input '.$checked.'type="checkbox" class="stdModBox" name="' . htmlspecialchars($name) . '" />');
            $html->html('<span class="label"><strong>' . htmlspecialchars($language['title']) . '</strong></span><br />');
            foreach ($language['zones'] as $zoneKey => $zone) {
                $name = $prefix . '_' . $language['id']. '_' . $zoneKey . '_';
                $checked = '';
                ...
                foreach ($zone['options'] as $optionKey => $value) {
                    ...
                    $html->html($indent.'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input '.$checked.'type="checkbox" class="stdModBox" name="' . htmlspecialchars($name) . '" />');
                    $html->html('<span class="label">' . htmlspecialchars($label) . '</span>');
                    $html->html('<br />');
                }
            }
        ...
        if ($rs) {
            foreach ($this->options as $languageKey => $language) {
                if (isset($_REQUEST['' . $prefix . '_' . $language['id'] . '_'])) {
                    Db::insertLanguageRule($language['id'], (int) $rowId);
                }
        
                foreach ($language['zones'] as $zoneKey => $zone) {
                    if (isset($_REQUEST['' . $prefix . '_' . $language['id'] . '_' . $zoneKey . '_'])) {
                        Db::insertZoneRule($language['id'], $zoneKey, (int) $rowId);
                    }
                    foreach ($zone['options'] as $optionKey => $value) {
                        if (isset($_REQUEST['' . $prefix . '_' . $language['id']. '_' . $zoneKey. '_' . $value[0]])) {
                            Db::insertPageRule($language['id'], $zoneKey, $value[0], (int) $rowId);
                        }
    ...
    */
    private function _getOptions($parent = null, $prefix = '') {
        ...
        $tmpExcludedZones = explode("\n", $parametersMod->getValue('community', 'restrictions', 'options', 'excluded_zones'));
        $excludedZones = array();
